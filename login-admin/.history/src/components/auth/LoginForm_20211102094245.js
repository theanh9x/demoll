...
const LoginForm = (props) => {
  ...
  const onFinish = (values) => {
    ...
    LoginService.apiJWTToken()
      .then((token) => {

        console.log(token.data.access_token);
        LoginService.signIn(values.username, values.password, token.data.access_token)
          .then((res) => {
            localStorage.setItem('access_token', JSON.stringify(res.data.accestoken));
